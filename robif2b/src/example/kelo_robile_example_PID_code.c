    ...
    while (true) {
        ...
        for (int i = 0;i<NUM_DRIVES;i++){
            ...
            if(setpoint[i] <  state.kelo_msr.pvt_pos[i] && state.kelo_msr.pvt_pos[i] < setpoint[i]+3.14){
                ...
                error[i] = setpoint[i] - state.kelo_msr.pvt_pos[i];
                
                printf("Error value %d: %f \n", i, error[i]);

                if (0.0 < error[i] && error[i] < error_threshold){
                ...
                // Kp gain = 0.5
                torque_value = error[i]*kp_gain;

                printf("Torque value %d: %f \n", i, torque_value);

                state.kelo_cmd.trq[2*i] = torque_value; // clockwise
                state.kelo_cmd.trq[2*i+1] = torque_value;
                ...
                printf("!!!CLOCKWISE %d \n", i);
                    ...
                    error[i] = 2*pi + (setpoint[i] - state.kelo_msr.pvt_pos[i]);
                    ...
                    torque_value = -1*error[i]*kp_gain; //-1 to rotate in anti-clockwise

                    printf("\n Torque value %d: %f \n", i, torque_value);

                    state.kelo_cmd.trq[2*i] = torque_value; // anti-clockwise
                    state.kelo_cmd.trq[2*i+1] = torque_value;
