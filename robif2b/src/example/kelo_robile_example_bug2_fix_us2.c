...
// int main()
{
    ...
    // Pivot angle offset for positive X-direction
    // NOTE: measured manually
    // state.kelo_msr.pvt_off[0] = 2.10;
    // state.kelo_msr.pvt_off[1] = 3.47;
    // state.kelo_msr.pvt_off[2] = 2.94;
    // state.kelo_msr.pvt_off[3] = 0.60;
    ...
    if(angle == 0){
        ...
        // POS Y
        setpoint[0] = fmod(2.10 + M_PI/2, max_angle);
        setpoint[1] = fmod(3.47 + M_PI/2, max_angle);
        setpoint[2] = fmod(2.94 + M_PI/2, max_angle);
        setpoint[3] = fmod(0.60 + M_PI/2, max_angle);


    }else if(angle ==180){
        
        // NEG X
        setpoint[0] = fmod(2.10 + M_PI, max_angle);
        setpoint[1] = fmod(3.47 + M_PI, max_angle);
        setpoint[2] = fmod(2.94 + M_PI, max_angle);
        setpoint[3] = fmod(0.60 + M_PI, max_angle);

    }else if(angle == 270){
        
        // NEG Y
        setpoint[0] = fmod(2.10 + 1.5 * M_PI, max_angle);
        setpoint[1] = fmod(3.47 + 1.5 * M_PI, max_angle);
        setpoint[2] = fmod(2.94 + 1.5 * M_PI, max_angle);
        setpoint[3] = fmod(0.60 + 1.5 * M_PI, max_angle);
    ...
    while (true){
        ...
        for (int i = 0;i<NUM_DRIVES;i++){
            ...
            if (isAligned == false){
                ...
                if (setpoint[i] > M_PI){
                    setpoint[i] = setpoint[i] - 2 * M_PI;

                    if (state.kelo_msr.pvt_pos[i] > M_PI){
                        measurement_value = state.kelo_msr.pvt_pos[i] - 2 * M_PI;

                        if (measurement_value > setpoint[i]){
                            
                            state.kelo_cmd.trq[2*i] = 1.3; // clockwise
                            state.kelo_cmd.trq[2*i+1] = 1.3;
                            printf("!!!CLOCKWISE Wheel Unit %d \n", i);

                        }else{

                            state.kelo_cmd.trq[2*i] = -1.3; //counterclockwise
                            state.kelo_cmd.trq[2*i+1] = -1.3;
                            printf("!!!ANTI-CLOCKWISE Wheel Unit %d\n", i);

                        }

                    }else if (0 <= state.kelo_msr.pvt_pos[i] &&  
                            state.kelo_msr.pvt_pos[i] <= M_PI){

                        if (setpoint[i] < state.kelo_msr.pvt_pos[i] &&
                            state.kelo_msr.pvt_pos[i] < setpoint[i] + M_PI){
                        ...
                        if (state.kelo_msr.pvt_pos[i] > M_PI){
                            ...
                            if (measurement_value < setpoint[i] - M_PI){
            ...
            // Stop wheel if it is aligned
            // 0.05 is the acceptable error range (in radians), or ~3 in degrees
            if (setpoint[i]-0.05 < state.kelo_msr.pvt_pos[i] && 
                state.kelo_msr.pvt_pos[i] < setpoint[i]+0.05){ 
        ...
        if (stop_wheel_counter[0] == 1 && 
            stop_wheel_counter[1] == 1 && 
            stop_wheel_counter[2] == 1 && 
            stop_wheel_counter[3] == 1){
