...
// For four active wheels configuration
#define NUM_DRIVES 4
#define NUM_SLAVES 5
...
int main(int argc, char *argv[])
{
    ...
    // changed id mapping for force distribution code
    ...
    // gsl_matrix_set(b, 1, 0, -100.); // force is set in Y-direction
    ...
    double f_max = 4.0;
    double f_var = 0;
    double vel_threshold = 30; // base:15.30
    double damping_const = f_max / vel_threshold;
    double stop_flag = 0;
    double avg_iterations = 30; // base:8,7
    double cnt_iter = 0;
    double vel_acc = 0;
    ...
    while (true)
    {
        ...
        */
        for (int i = 0; i < NUM_DRIVES; i++)
        {
            ...
            {
                printf("!!!wheel unit %d is not aligned\n", i);
        ...
        /*
        If platform aligned then only apply force to wheels
        */
        ...
        if (isAligned == true)
        {
            ...
            for (int i = 0; i < NUM_DRIVES; i++)
            {
                ...
                state.kelo_cmd.trq[2 * i] = -f_var;
                state.kelo_cmd.trq[2 * i + 1] = f_var;

                /**
                 * @brief keeping the max velocity of the wheels below a threshold
                 *
                 */

                if ((((
                          (fabs(state.kelo_msr.whl_vel[0]))) > vel_threshold)) &&
                    ((((fabs(state.kelo_msr.whl_vel[2]))) > vel_threshold)) && ((((fabs(state.kelo_msr.whl_vel[4]))) > vel_threshold)) && ((((fabs(state.kelo_msr.whl_vel[6]))) > vel_threshold)))
                {

                    // check velocity for # iterations
                    if (cnt_iter > avg_iterations)
                    {
                        if (stop_flag == 0)
                        {
                            f_var = 0;
                            stop_flag = 1;
                            printf("\nROBOT stopped after %f iterations\n", avg_iterations);
                        }
                    }
                    else
                    {
                ...
                {
                    f_var = f_max - damping_const * ((fabs(state.kelo_msr.whl_vel[i])));
                    vel_acc = 0;
                }
            ...
            // Debug: To give linear motion to the robot

            // state.kelo_cmd.trq[0] = -f_var;
            // state.kelo_cmd.trq[1] = f_var;
            // state.kelo_cmd.trq[2] = -f_var;
            // state.kelo_cmd.trq[3] = f_var;
            // state.kelo_cmd.trq[4] = -f_var;
            // state.kelo_cmd.trq[5] = f_var;
            // state.kelo_cmd.trq[6] = -f_var;
            // state.kelo_cmd.trq[7] = f_var;
        }
